...
class Bola:
    ...
    def mover(self):
        ...
        if self.x - self.raio <= 0 or self.x + self.raio >= 800:
            if self.x - self.raio<0: # n deixa entrar na parede
                self.x = self.raio # n deixa entrar na parede
            if self.x + self.raio>=800:# n deixa entrar na parede
                self.x= 800-self.raio# n deixa entrar na parede
        ...
        if self.y + self.raio>600: #se bater no chao
            ...
            self.velocidade_y *= -1 #reseta altura/pulo sem perder velocidade
            #self.mudarcor()
    def verifica_colisao(self,outra_bola):
        ...
        if distancia <= self.raio + outra_bola.raio:
            if self.x>outra_bola.x: # n deixa entra uma na outra
                self.x+=2 
                outra_bola.x-=2
            if self.y>outra_bola.y: # n deixa entra uma na outra
                self.y+=2
                outra_bola.y-=2 
